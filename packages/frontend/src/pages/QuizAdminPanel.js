    ...
    const getUserAnswers = async () => {
        ...
        try {
            ...
            responseCourses.data.userCourses.map(userCourse => {
                ...
                userListTemp.push( { "userId" : userCourse.user.id, "userName" : userCourse.user.email})
                            ...
                            <tbody>
                                {userList.map((user, i) => (
                                    <tr className={`quiz-tr-${i%2}`}>
                                        <th scope="row">{i + 1}</th>
                                        <td>{user.userName}</td>
                                        <td>{userPoints[i] !== "NaN" ? `${parseFloat(userPoints[i]).toFixed(2)} / ${questions}`: "Nie przystąpił"}</td>
